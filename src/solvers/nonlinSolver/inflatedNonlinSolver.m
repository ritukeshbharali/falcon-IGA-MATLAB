% Inflated Newton-Raphson solver 

function globdat = inflatedNonlinSolver(props,globdat)
...
while true
    ...
    % Iteration counter increment, and store old internal force
        ...
        if globdat.ts.step > 11
            [~, tau]        = choleskyMultIdentity(globdat.K);
            globdat.K       = globdat.K + tau * eye(globdat.ndofs);
        end
