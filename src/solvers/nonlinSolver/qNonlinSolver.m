%=========================================================================%
% quasi-Monolithic (Newton-Raphson) solver [Heister et. al. (2015)]
%=========================================================================%

function globdat = qNonlinSolver(props,globdat)
...
globdat.ts.dt00 = globdat.ts.dt0;
globdat.ts.dt0  = globdat.ts.dt;

fprintf('\n')
disp('========================================================')
disp(['Step ', num2str(globdat.ts.step),', Step-size: ',...
      num2str(globdat.ts.dt),', Method: quasi Monolithic'])
disp(['Elements = ',num2str(globdat.mesh.numElements),...
      ', DOFs = ',num2str(globdat.ndofs)])
disp('========================================================')
disp(['Iteration    | ','Error'])

% Create a vector to store error
errVec = zeros(props.nlSolver.maxiter,1);

% Compute extrapolated solution
globdat.Exstate = globdat.state00 + ...
            (globdat.ts.dt00+globdat.ts.dt0)/globdat.ts.dt00 * ...
...
while true
    ...
    % Iteration counter increment, and store old internal force
        ...
        globdat         = ModelAction(props,globdat,'GET_MATRIX0_EXT');
        ...
        if err < props.nlSolver.tol
            ...
            if sum([globdat.markRef{:}]) && props.geom.AMR
                ...
                globdat = ModelAction(props,globdat,'REFINE_N_TRANSFER');
