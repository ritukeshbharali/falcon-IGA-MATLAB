%=========================================================================%
% postProcessStep (carries out post-processing step after a solution has
%                  converged) 
% 
% Available options: 
%                  * real-time plots every 'n' steps
...
if rem(globdat.ts.step,props.postProc.onScreen) == 0 || ...
    ...
    dispcoord  = real(dispcoord);
    physcoord  = real(physcoord);
    wcoord     = real(wcoord);
    ...
    if rem(globdat.ts.step,props.postProc.onScreen) == 0 || ...
        ...
        cla(plot4)
        factor = 0;
        trisurf(vIEN,physcoord(:,1)+dispcoord(:,1)*factor, physcoord(:,2)+dispcoord(:,2)*factor, zeros(size(physcoord,1),1), real(wcoord), 'facecolor','interp', 'EdgeColor','none')
        view(0,90)
        title('Phase Field')
        colorbar('vert')
        colormap('parula')
